...
function pick(pkg: PackageJSON): Record<string, unknown> {
...
function mergeContributes(pkg: PackageJSON): Record<string, unknown> {
  ...
  for (const [k, v] of Object.entries(sumi)) {
    if (Array.isArray(v) && Array.isArray(out[k])) {
      out[k] = [...(out[k] as unknown[]), ...(v as unknown[])];
...
async function main(): Promise<void> {
  ...
  for (const file of files) {
    ...
    const pkg = JSON.parse(pkgEntry.getData().toString('utf-8')) as PackageJSON;
    ...
    for (const entry of zip.getEntries()) {
      ...
      if (!target.startsWith(path.resolve(outDir))) continue; // 防路径穿越
