 ...
 *
 * RemoteFS: 读写全透传 server fs 的 BrowserFS 后端（opensumi 容器专用适配层）.
 *   - opensumi 容器（explorer/编辑器/@ 引用）经 BrowserFS 访问文件系统
 *   - 所有操作直连 service/fs 单实例（业务代码与容器共用同一文件系统实例）→ server /fs/*
...
/** server FileMeta → BrowserFS Stats（mtime 取 server 值: 保存时 lastModification 稳定, 不误报 out-of-sync） */
...
export class RemoteFS extends BaseFileSystem {
  ...
  stat(p: string, _isLstat: boolean | null, cb: Cb<Stats>): void {
    getFileSystemService()
      ...
      .catch(() => {
        // 未就绪（登录前 fsUrl 未注入）或不存在: 根目录视为空目录, 避免 file-tree 判定"无打开的文件夹";
        // 登录后 fsUrl 就绪 + explorer 刷新, 自动填充真实内容
  ...
  open(p: string, flags: FileFlag, _mode: number, cb: Cb<NoSyncFile<RemoteFS>>): void {
    // 写路径由 writeFile 直连 server 处理; open 仅服务读
    if (flags.isWriteable()) {
      return cb(ApiError.EPERM(p));
    }
    const rel = sandboxRel(p);
  ...
  writeFile(fname: string, data: unknown, _encoding: string | null, _flag: FileFlag, _mode: number, cb: Cb): void {
    ...
    const rel = sandboxRel(fname);
    console.log('[bfs] writeFile → server:', rel, JSON.stringify(content.slice(0, 40)));
    getFileSystemService()
      .write(rel, content)
      .then(() => cb(null))
  ...
  rename(oldPath: string, newPath: string, cb: Cb): void {
    getFileSystemService()
      .move(sandboxRel(oldPath), sandboxRel(newPath))
      .then(() => cb(null))
