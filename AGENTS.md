...
| **PDF 标注改 AI ask + 动画演示** | 旧标注是"交互三选表单" (comment/prompt/file); rect 圈选 fixed 定位随滚动跑 | ①rect 圈定即锚定页面内 (absolute) ②popover 改 AI ask 风格 (提示词 + 生成按钮, 交互能力可扩展注册表, 本次 demo) ③生成动画演示: ask → 保存 {pdf}-动画演示-{n}.html (PDF 同目录, AI 写文件时读回兜底) → sidecar interactions 加 demo → rect 右下角按钮打开; sidecar 路径改 PDF 同目录 + WORKSPACE_ROOT URI | `web/src/extensions/pdf/` + `docs/标注功能设计与测试用例.md` (2026-09-01) |
| **explorer 移动大文件损坏** | fileService.move → codeblitz disk-file-system.provider.doMove → `fse.move` (fs-extra = **copy+remove** 实现!) — 30MB+ PDF 经 SDK 读 + 分块写损坏 (实测 30MB → 262KB); OverlayFS.renameSync 文件分支同样 copy | postinstall patch 两处: ①disk-provider.doMove `fse.move` → 桥接 `window.__APP_FS__.move` (FsPty 宿主机原子 mv, 不可用兜底原实现) ②OverlayFS.renameSync readable-only → 写 `/.browserfs_moves.log` → runtime.ts `_syncSync` 拦截 → syncMove; 验证: 移动后 md5 与源一致 | `web/scripts/patch-codeblitz-constant.js` + `web/src/config/runtime.ts` (2026-09-01) |
