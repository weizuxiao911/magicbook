...
export const WorkspacePicker: React.FC = () => {
  ...
  /** 错误提示统一走 framework notification (右下角 toast, 自动消失) */
  const notifyError = useCallback((msg: string) => {
    notification.error({ message: msg, type: 'error', duration: 3 });
  }, []);

  const doBrowse = useCallback(async (dir: string, isRetry = false) => {
    if (!dir.trim()) return;
    setLoading(true);
    try {
      ...
      // 不清空 dirs / currentPath — 保留上一次成功的列表, 用户能继续浏览
    }
    finally { setLoading(false); }
  }, []);

  /** recent 路径点击: 先 doBrowse 校验存在, 成功才 setCwd (避免 reload 后才发现 stale) */
  const selectRecent = useCallback(async (p: string) => {
    setLoading(true);
    try {
      await browseDir(p);
      setCwd(p);
    } catch (e: any) {
      notifyError(`路径不存在或不可访问: ${p}`);
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    const h = async () => {
      setOpen(true); setDirs([]); setSelected(null); setMkMode(false); setMkName('');
      ...
      <div className="wp-modal">
        ...
        <div className="wp-body">
            ...
            <div className="wp-side">
              ...
              {recent
                ...
                .map((p) => {
                  ...
                  return (
                    <div key={p} className="wp-side-item wp-side-item--recent" title={p}>
                      <button
                        type="button"
                        className="wp-side-item-main"
                      ...
                      >
                        ...
                        <span className="wp-side-item-name">{name}</span>
                      </button>
                      <button
                        type="button"
                        className="wp-side-item-remove"
                        title="从最近移除"
                        onClick={(e) => { e.stopPropagation(); removeRecent(p); }}
                      >
                        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
                      </button>
                    </div>
                  );
                })}
          ...
          <div className="wp-main" onContextMenu={(e) => { e.preventDefault(); setCtxMenu({ x: e.clientX, y: e.clientY }); }}>
            <div className="wp-main-tools">
              <div className="wp-search">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg>
                <input
                  ref={searchRef}
                  type="text"
                ...
                />
                {query && (
                  <button type="button" className="wp-search-clear" title="清空" onClick={() => { setQuery(''); searchRef.current?.focus(); }}>
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
                  </button>
                )}
              </div>
              <button type="button" className="wp-mk-btn-top" title="新建目录" onClick={openMkdir}>
            ...
            {!loading && dirs.length === 0 && <div className="wp-empty">空目录</div>}
            {!loading && dirs.length > 0 && (() => {
              const q = query.trim().toLowerCase();
              const filtered = q ? dirs.filter((d) => d.name.toLowerCase().includes(q)) : dirs;
              if (filtered.length === 0) return <div className="wp-empty">无匹配 "{query}"</div>;
              return (
                <div className="wp-list">
                  {filtered.map((entry) => {
                    const i = dirs.indexOf(entry);
                    return (
...
.wp-side-item--recent{font-size:12px;padding:0;gap:0}
.wp-side-item-main{display:flex;align-items:center;gap:8px;flex:1;min-width:0;padding:6px 10px;background:none;border:none;color:inherit;font:inherit;text-align:left;cursor:pointer;border-radius:6px 0 0 6px}
.wp-side-item-main:hover{background:var(--ai-hover,rgba(255,255,255,0.05));color:var(--ai-fg,#e5e7eb)}
.wp-side-item-name{flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.wp-side-item-remove{width:22px;height:100%;padding:0;background:none;border:none;color:var(--ai-fg-muted);cursor:pointer;opacity:0;border-radius:0 6px 6px 0;display:flex;align-items:center;justify-content:center}
.wp-side-item--recent:hover .wp-side-item-remove{opacity:.7}
.wp-side-item-remove:hover{opacity:1 !important;color:#fca5a5;background:rgba(239,68,68,0.08)}

/* Main */
.wp-main{flex:1;overflow-y:auto;padding:8px 10px;display:flex;flex-direction:column;gap:6px;min-height:0}
.wp-main-tools{display:flex;align-items:center;gap:8px;flex-shrink:0;padding:4px 0}
.wp-search{flex:1;display:flex;align-items:center;gap:8px;padding:10px 14px;background:rgba(255,255,255,0.04);border:1px solid var(--ai-divider,rgba(255,255,255,0.08));border-radius:8px;color:var(--ai-fg-muted,#6b7280);height:38px}
.wp-search:focus-within{border-color:var(--ai-accent,#6366f1);color:var(--ai-fg,#e5e7eb)}
.wp-search-inp{flex:1;background:none;border:none;outline:none;color:inherit;font-size:13.5px;min-width:0;height:100%}
.wp-search-inp::placeholder{color:var(--ai-fg-muted,#6b7280)}
.wp-search-clear{background:none;border:none;color:inherit;cursor:pointer;padding:0;display:flex;align-items:center;opacity:.6}
.wp-search-clear:hover{opacity:1}
.wp-mk-btn-top{width:32px;height:32px;padding:0;background:var(--ai-hover,rgba(255,255,255,0.05));border:1px solid var(--ai-divider,rgba(255,255,255,0.08));border-radius:8px;color:var(--ai-fg-muted,#cbd1d8);cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0}
.wp-mk-btn-top:hover{background:var(--ai-accent,#6366f1);color:#fff;border-color:var(--ai-accent,#6366f1)}
